...
cout<<"ingrese ael carnet a eliminar:";
cin>>buscadoe;

if (archivo.is_open() && temp.is_open()) {
while (archivo>>e.nombre>>e.carnet >>e.edad) {
if (e.carnet != buscadoe) {
    temp << e.nombre << " " << e.carnet << " " << e.edad << endl;
}else{
     eliminado=true;   
}
...
remove ("temp.txt, estudiante.txt");//subtituyo estudiantes txt con el temporal para que asi lo elimine y se guarde el otro cambio 

if (eliminado )
cout<<"el estudiante fue eliminado"<<endl;
else
